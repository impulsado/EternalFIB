...
void bfs(const VVC& tauler, VVI& vists, const PII& pos_inicial) {
    ...
    if (found) {
        int x = pos_final.first, y = pos_final.second;
        llista_vertexs.push_back(pos_final);
        while (vertex_previ[x][y].first != -1 && vertex_previ[x][y].second != -1) {
            llista_vertexs.push_front(vertex_previ[x][y]);
            x = vertex_previ[x][y].first;
            y = vertex_previ[x][y].second;
        }
...
int main(void) {
    ...
    while (cin >> usr_num_rows >> usr_num_cols) {
        ...
        VVI vists(usr_num_rows, VI(usr_num_cols));
        PII pos_inicial;
