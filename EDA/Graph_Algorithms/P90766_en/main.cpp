...
void checkMovments(vector<pair<int,int>>& movments, const vector<vector<char>>& M, const vector<vector<bool>>& visited, int x, int y) {
    if (x-1>=0 && !visited[x-1][y] && M[x-1][y] != 'X') {  // AMUNT
        pair<int,int> temp;
        temp.first = x-1;
        temp.second = y;
        movments.push_back(temp);
    }
    if (y-1>=0 && !visited[x][y-1] && M[x][y-1] != 'X') {  // ESQUERRA
        pair<int,int> temp;
        temp.first = x;
        temp.second = y-1;
        movments.push_back(temp);
    ...
    if (y+1<M[0].size() && !visited[x][y+1] && M[x][y+1] != 'X') {  // DRETA
        pair<int,int> temp;
        temp.first = x;
        temp.second = y+1;
        movments.push_back(temp);
    ...
    if (x+1<M.size() && !visited[x+1][y] && M[x+1][y] != 'X') {  // AVALL
        pair<int,int> temp;
        temp.first = x+1;
        temp.second = y;
        movments.push_back(temp);
...
int main() {
    ...
    findTreasure(M, visited, c-1, r-1);
    cout << counter << endl;
