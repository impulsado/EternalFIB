    ...
    while (cin >> command) {
        if (command == "v.push_back(") {
            ...
            if (newsize < currentSize) {
                int new_last_index = first_index+newsize;
                auto new_last_it = values.lower_bound(new_last_index);
                values.erase(new_last_it,values.end());  // [new_last_it,end]
            ...
            auto it = values.find(first_index);
            if (it != values.end()) {
                cout << it->second;
                ++it;
            }
            ...
            for (int i = first_index+1; i<=last_index; i++) {
                if (it!=values.end() and it->first==i) {
                    cout << ',' << it->second;
                    it++;
                }
                else cout << ',' << 0;
