...
bool customFilter(Estudiant a, Estudiant b) {
...
int main () {
    ...
    // Ordenar per nota Desc o sino DNI asc
    sort(estudiants.begin(), estudiants.end(), customFilter);

    // Mostrar Resultats
    for (int i = 0; i<num_est; i++) cout << estudiants[i].consultar_DNI() << ' ' << estudiants[i].consultar_nota() << endl;
}