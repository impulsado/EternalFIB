...
public:
  ...
  void moveTowardsEnd(iterator &it) {
    ...
    // És el primer
    if (it.pitem==iteminf.next) {
      it.pitem->next->next->prev = it.pitem;
      Item* aux = it.pitem->next;
      it.pitem->next = it.pitem->next->next;
      aux->next = it.pitem;
      iteminf.next = aux;  // !
      aux->prev = it.pitem->prev;
      it.pitem->prev = aux;
    }
    // No és el primer
    else {
